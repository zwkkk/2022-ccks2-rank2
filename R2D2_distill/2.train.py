...
from align_model import R2D2ForAlign, R2D2CrossForAlign, R2D2FullCrossForAlign, R2D2DistillForAlign
from typing import Dict
from sklearn.metrics import f1_score, precision_score, recall_score
from transformers.trainer_utils import EvalPrediction
from dataset import get_model_data, R2D2PairDataset, R2D2DistillDataset, r2d2_pair_collate_fn, get_model_data_v2, R2D2CrossDataset, r2d2_cross_collate_fn, r2d2_distill_collate_fn
...
def load_pytorch_model(model_path, model, strict=True):
...
def load_model(self):
        ...
        if self.checkpoint_path_bi.endswith(".bin"):
            weight_path = self.checkpoint_path_bi
        else:
            weight_path = os.path.join(self.checkpoint_path_bi, "pytorch_model.bin")
        weight = torch.load(weight_path)
        self.S_model.load_state_dict(weight, strict=True)
...
def run():
    ...
    train_dataset = R2D2DistillDataset(train_texts1, train_texts2, train_img_paths_1, train_img_paths_2, train_labels, max_length=200)
    test_dataset = R2D2DistillDataset(test_texts1, test_texts2, test_img_paths_1, test_img_paths_2, test_labels, max_length=200)
    model = R2D2DistillForAlign("../R2D2_cross/pretrain_checkpoints/last/pytorch_model.bin", "../R2D2_cross/align_checkpoints/best/pytorch_model.bin")
    ...
    trainer = MyTrainer(model=model,
                        ...
                        data_collator=r2d2_distill_collate_fn,
