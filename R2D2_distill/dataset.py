...
def get_pretrain_data_v4():
    ...
    pair_self1 = [t1+"[SEP]"+t1 for t1 in texts]
    texts = texts + pair_texts + pair_self1
    pair_img_paths = [(i1, i2) for i1,i2,label in zip(img_paths_1, img_paths_2, labels) if label==1]
    pair_img_paths_self1 = [(i1, i1) for i1 in img_paths]
    img_paths = img_paths + pair_img_paths + pair_img_paths_self1
    
    print(len(texts1))
    print(len(pair_self1))
...
def get_pretrain_data():
    ...
    test_items = read_data("../data/item_test_info.jsonl")
    ...
    test_items["image_path"] = test_items["item_image_name"].apply(lambda i_name: os.path.join("../data/item_test_images/", i_name))
    items = pd.concat([train_items, valid_items, test_items])
    text_feature_file = "../data/item_pretrain_text_title.pkl"
    item2text = pickle.load(open(text_feature_file, "rb"))
    texts = items["item_id"].apply(lambda item_id: item2text[item_id]).tolist()
...
def get_model_data(dname: str, return_ids: bool = False):
    if dname=="valid":
        ...
        text_feature_file = "../data/item_test_text_title.pkl"
        item_info_file = "../data/item_test_info.jsonl"
        images_file = "../data/item_test_images/"
    elif dname=="season1":
        text_feature_file = "../data/item_valid_text_title.pkl"
        item_info_file = "../data/item_valid_info.jsonl"
        images_file = "../data/item_valid_images/"
    else:
        text_feature_file = "../data/item_train_text_title.pkl"
    ...
    item2text = pickle.load(open(text_feature_file, "rb"))
    if dname=="train":
        items1 = read_data(item_info_file)
        items1["image_path"] = items1["item_image_name"].apply(lambda i_name: os.path.join(images_file, i_name))
        items1.index = items1["item_id"]
        item2imgpath1 = items1["image_path"].to_dict()
        items2 = read_data("../data/item_test_info.jsonl")
        items2["image_path"] = items2["item_image_name"].apply(lambda i_name: os.path.join("../data/item_test_images/", i_name))
        items2.index = items2["item_id"]
        item2imgpath2 = items2["image_path"].to_dict()
        item2imgpath = {**item2imgpath1, **item2imgpath2}
    ...
    if dname=="train":
        ...
        data=read_data("../data/item_train_pair_train_plus.jsonl")
    elif dname=="season1":
        data=read_data("../data/item_valid_pair.jsonl")
    elif dname=="test":
        data=read_data("../data/item_train_pair_test.jsonl")
    elif dname=="valid":
        data=read_data("../data/item_test_pair.jsonl")
    elif dname=="retrain":
        data=read_data("../data/item_train_pair_train_plus.jsonl")
...
class R2D2DistillDataset(Dataset):
    def __init__(self,
                 texts1: List[str],
                 texts2: List[str],
                 img_paths_1: List[str],
        ...
        for idx in tqdm(range(len(texts1))):
            ...
            label = labels[idx]
            # pair dataset
            text1_inp = tokenizer(text1, padding="max_length", truncation=True, max_length=75, return_tensors="pt")
            text2_inp = tokenizer(text2, padding="max_length", truncation=True, max_length=75, return_tensors="pt")
            text1_inp = {"src_"+k:v for k,v in text1_inp.items()}
            text2_inp = {"tgt_"+k:v for k,v in text2_inp.items()}
            inp.update(text1_inp)
            inp.update(text2_inp)
...
def r2d2_distill_collate_fn(features: List[Dict]):
    # text
    batch_src_input_ids = [feature["src_input_ids"] for feature in features]
    batch_src_attention_mask = [feature["src_attention_mask"] for feature in features]
    batch_src_token_type_ids = [feature["src_token_type_ids"] for feature in features]
    ...
    return {"src_input_ids": batch_src_input_ids,
            ...
            "labels": batch_label,
            "input_ids": batch_cross_input_ids,
            "attention_mask": batch_cross_attention_mask,
            "token_type_ids": batch_cross_token_type_ids,
            "pixel_value": batch_cross_pixel_value}
...
if __name__ == "__main__":
    ...
    dataset = R2D2DistillDataset(texts1, texts2, img_paths_1, img_paths_2, labels, max_length=256)
