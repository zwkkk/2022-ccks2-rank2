...
class R2D2CrossForAlign(nn.Module):
    def __init__(self, checkpoint_path: str = None):
        ...
        self.src_proj = MLPProjector(768, 768)
        self.tgt_proj = MLPProjector(768, 768)
        self.sim = Similarity()
...
class R2D2FullCrossForAlign(nn.Module):
    ...
    def train_forward(self,
        ...
        cls2 = text_hidden_state2[:, 0, :]
        src_cls2 = self.src_proj(cls2)
        tgt_cls2 = self.tgt_proj(cls2)
        src_cls2 = src_cls2/src_cls2.norm(dim=-1, keepdim=True)
        tgt_cls2 = tgt_cls2/tgt_cls2.norm(dim=-1, keepdim=True)
        cos_sim2 = self.sim(src_cls2, tgt_cls2)
        
        rdrop_loss = F.mse_loss(cos_sim,cos_sim2)
        rdrop_loss = 4 * rdrop_loss.sum()
        #print("normal loss 1: {}".format(F.mse_loss(cos_sim,labels.float())))
        #print("normal loss 2: {}".format(F.mse_loss(cos_sim2,labels.float())))
        #print("rdrop loss: {}".format(rdrop_loss))
        loss = F.mse_loss(cos_sim,labels.float()) + F.mse_loss(cos_sim2,labels.float()) + rdrop_loss
