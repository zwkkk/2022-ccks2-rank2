...
class R2D2ForPretrain(nn.Module):
    ...
    def forward(self,
        ...
        if dist.is_initialized() and self.training:
            ...
            z1_list = [torch.zeros_like(text_embed) for _ in range(dist.get_world_size())]
            z2_list = [torch.zeros_like(image_embed) for _ in range(dist.get_world_size())]
            
            # Allgather
            dist.all_gather(tensor_list=z1_list, tensor=text_embed.contiguous())
            dist.all_gather(tensor_list=z2_list, tensor=image_embed.contiguous())

            # Since allgather results do not have gradients, we replace the
            # current process's corresponding embeddings with original tensors
            z1_list[dist.get_rank()] = text_embed
            z2_list[dist.get_rank()] = image_embed
        
            # Get full batch embeddings: (bs x N, hidden)
            text_embed_whole = torch.cat(z1_list, 0)
            image_embed_whole = torch.cat(z2_list, 0)
        '''
